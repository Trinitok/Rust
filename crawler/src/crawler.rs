	...
	url_states: Sender<UrlState>,
		...
		loop{
...

			{
				...
				//  see if place we are trying to go to has been visited
				if visited_val.contains(&current) {  //  string we are trying to go to
					let mut active_count_val = active_count.lock().unwrap();
					*active_count_val -= 1;
					continue;
				}
...
pub fn crawl(domain: &str, start_url: &Url) -> Crawler{
	//  putting url into vector as a string
	...
	//  create threads and send them out
	for _ in 0..THREADS{
