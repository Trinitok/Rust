...
use std::io::Read;
use std::thread;  // used for multi threading
use std::time::Duration;
use std::sync::mpsc::channel;  // push items through channels
use std::fmt;

use self::hyper::Client;  //  connecting to things
use self::hyper::status::StatusCode;  //  get back html codes
use self::url::{ParseResult, Url, UrlParser};  //  parse request results for calling urls
...
#[derive(Debug, Clone)]
pub enum UrlState{
	Accessible(Url),  // success
	BadStatus(Url, StatusCode),  // link not work
	ConnectionFailed(Url),  // connection not working
	TimedOut(Url),  // time out failure
	Malformed(String),
}

//  handle what is displayed based on what is returned
impl fmt::Display for UrlState{
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self{
			...
			UrlState::TimedOut(ref url) => format!("x {} (timeout)", url).fmt(f),
...
pub fn url_status(domain: &str, path: &str) -> UrlState {
	match build_url(domain, path){
		Ok(url) => {
			let (tx, rx) = channel();  // tx = transmitter
			let req_tx = tx.clone();
			let u = url.clone();
